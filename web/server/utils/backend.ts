...
/**
 * Refresh token 的 cookie 名稱。 存在 httpOnly cookie 而非 localStorage：後者是 XSS 的直接目標， 任何被注入的第三方腳本都能讀走它。httpOnly 讓 JS 完全碰不到。
 */
...
/**
 * 寫入 refresh token cookie。 sameSite 用 'lax' 而非 'strict'：strict 會讓使用者從外部連結進站時 帶不上 cookie，看起來像被登出。lax 已經擋掉 CSRF 的主要向量（跨站 POST）， 而本站的寫入操作又都需要 Authorization 標頭——那是 cookie 帶不上的東西。
 */
...
/** 只回傳 access token 與其效期給瀏覽器。 refresh token 刻意不在回應中——它只存在於 httpOnly cookie 裡。 */
