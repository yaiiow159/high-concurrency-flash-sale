...
/**
 * 一則評價。 **頭像用名字的第一個字，不是灰色人形圖示。** 一整排相同的灰人形會讓評價區看起來像是機器產生的； 用首字加上由作者名決定的底色，每一則就有了辨識度， 而那正是「這是真人寫的」這個印象的來源。 色相由名字雜湊而來——同一個作者永遠是同一個顏色。 **「已編輯」要標出來。** 讀者有權知道他看的不是原始版本。 藏起來省不了什麼，被發現卻會直接損害整個評價區的可信度。
 */
...
/**
 * 頭像底色。 用固定的六個色相而不是 `hsl(hash % 360)`：繞色環算出來的顏色 十之八九是濁的，而且相鄰的兩則評價很容易撞成同一種灰紫。 這與 `ProductTile` 的色盤是同一個判斷。
 */
const AVATAR_HUES = [
