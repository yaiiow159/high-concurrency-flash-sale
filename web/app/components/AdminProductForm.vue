...
/**
 * 建立商品。 **規格至少一列，而且刪不到零。** 沒有 SKU 的商品不是「還沒建完」， 而是一個永遠不會出現在任何地方的殘骸——列表要顯示最低價， 而最低價來自 SKU（ADR-0015 決策 5）。表單一開始就給一列， 只剩一列時隱藏刪除鈕，讓「零個規格」在介面上不可能發生。 規格屬性用**鍵值對**而不是固定欄位：不同品類的規格維度本來就不同， 把「容量」寫成欄位的那一刻，賣衣服就得改 schema。
 */
...
/**
 * 每一列規格都要：至少一組完整的屬性、以及一個大於 0 的價格。 在**送出前**檢查而不是等後端回 400：後端的錯誤訊息只能說 「第幾個規格有問題」，而使用者要自己數到第幾列。 這不是把驗證搬到前端——後端那一份仍然是權威，這裡只是讓錯誤早一點被看見。
 */
