...
/**
 * 後台的狀態分頁。 用 `role="tablist"` 而不是一排 button：這是「同一份清單的不同切面」， 而不是五個獨立的動作。螢幕閱讀器會因此念出「第 2 個，共 4 個」， 鍵盤使用者也知道左右鍵能切換。 **不做「全部」這個分頁。** 後台的每一種狀態對應一種待辦， 混在一起之後就沒有任何一列是「該處理的」——那正是工作佇列 退化成資料表的方式。
 */
defineProps<{
