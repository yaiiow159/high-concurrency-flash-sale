...
/**
 * 後台總覽的一格待辦。 **數字為 null 時顯示破折號，不是 0。** 「查不到」與「沒有待辦」是完全不同的兩件事——把前者顯示成後者， 維運人員會以為今天沒事，而真相是那支 API 掛了。 這與 `MoneyText` 不把 null 顯示成 0 是同一個判斷。 `alert` 為真時整張卡換色。用**邊框與底色**而不是只換數字顏色： 這一頁的用途是掃過去找該處理的東西，而人掃版面時看的是塊，不是字。
 */
