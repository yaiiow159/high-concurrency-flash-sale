...
/**
 * 評分摘要：大分數 + 星等 + 分佈長條。 這是電商商品頁的固定語彙，照著做而不是自創——使用者已經知道怎麼讀它， 換個排法只會讓他們多花時間。三個部分各自回答一個問題： 分數說「好不好」、則數說「這個分數可不可信」、長條說「評價分不分歧」。 **則數與分數一樣重要。** 4.9 分（3 則）與 4.3 分（1,842 則） 是完全不同的兩件事，只顯示分數等於把後者的優勢藏起來。 長條的百分比由後端算好，前端不自己除——兩邊各算一次， 四捨五入遲早會不同，而那會表現成「長條加起來不是 100%」。
 */
defineProps<{
