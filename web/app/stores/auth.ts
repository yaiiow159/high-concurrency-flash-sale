...
/**
 * 認證狀態。 **access token 只放記憶體，刻意不持久化。** 放進 localStorage 是 XSS 的直接目標——任何被注入的第三方腳本都能讀走它。 放在記憶體裡，分頁一關就沒了，而重新整理後靠 httpOnly cookie 裡的 refresh token 自動續期即可，使用者無感。 refresh token 完全不在這裡，它只存在於 BFF 設下的 httpOnly cookie 中。
 */
export const useAuthStore = defineStore('auth', () => {
  ...
  /**
   * 這個人有沒有 admin 權限。 **只用來決定畫面上顯示什麼，永遠不是安全判斷。** 令牌是使用者手上的字串，他要改成什麼都行——但改完之後 打到後端仍然會被 `hasAuthority(SCOPE_ADMIN)` 擋下， 因為那一邊是驗過簽章的（ADR-0015 決策 2）。 讀 payload 而不是另外要一支 `/me`：scope 本來就在令牌裡， 多打一次 API 只是為了拿一個已經在手上的值。
   */
  ...
  /**
   * 續期，並確保併發呼叫只會實際發出一次請求。 這一點是必要的而非最佳化：後端每次續期都會**輪替** refresh token， 舊的立即失效。若三個請求同時 401 而各自發一次續期， 後兩次會拿著已失效的 token，反而觸發後端的重用偵測， 導致整條輪替鏈被撤銷、使用者莫名被登出。 與後端的 Redis 冪等是同一類問題：**併發下的重複請求要收斂成一次**。
   */
