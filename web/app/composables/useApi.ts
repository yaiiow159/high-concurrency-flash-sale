...
/**
 * 後端 API 的呼叫封裝。 集中處理三件事：帶令牌、401 時自動續期並重送、錯誤轉譯。 散落在各元件裡的話，遲早會有某個呼叫忘了處理 401， 表現成「偶爾要重新整理才能用」這種難查的問題。
 */
export function useApi() {
  ...
  /**
   * 把 fetch 的錯誤轉成帶錯誤碼的 ApiError。 後端所有端點都回相同的 `ApiResponse` 結構（含 401/403）， 因此這裡永遠拿得到錯誤碼——UI 才能區分「已售罄」與「系統忙碌」 而不必去比對訊息字串。
   */
