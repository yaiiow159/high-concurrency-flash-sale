...
/**
 * 退貨退款。 <b>可退數量一律問後端</b>，不在這裡自己扣。 「審核中的退貨單也佔用額度」是領域規則， 在前端複製一份的話兩邊遲早分岔，而分岔的症狀是 「畫面說可以退，送出卻被拒絕」——那是最難debug的一種前端 bug， 因為畫面看起來完全正常。
 */
export function useReturns() {
  ...
  /**
   * 送出退貨申請。 <b>冪等鍵由呼叫端在送出前產生並在重試之間保留</b>（見 return.vue）。 每次重試都換新值的話，網路逾時後再按一次，同一批商品就會被申請兩次退貨—— 而「累計退款 ≤ 已付金額」那道上限管的是金額總和不是每個品項的數量， 攔不到「退兩次 A、都不退 B」這種組合。
   */
  function open(
    orderNo: string,
  ...
  /** 撤回申請。 成功後重新載入整份清單而不是就地改狀態——與地址簿同樣的理由： 在前端推算後端會怎麼變，等於把不變式複製一份過來。 */
