...
/**
 * 結帳頁。 **不快取**——購物車與地址都是個資。 品項刻意**不由前端送出**：結帳請求只帶 requestId 與 addressId， 買什麼由伺服器從購物車讀。讓前端送品項等於讓它決定要買什麼， 那樣購物車就只是一個裝飾。
 */
...
/**
 * 伺服器算出來的金額明細。 前端**不自己算折扣**：門檻、上限、疊加順序都在後端， 兩邊各算一次遲早會得到不同答案，而使用者只會相信他先看到的那一個。 `null` 代表還沒試算完，此時沿用購物車的未折金額顯示—— 顯示 0 會讓畫面在載入時閃一下「免費」。
 */
...
/**
 * 這一次結帳的冪等鍵。 在進入頁面時就產生並保留，只有成功後才作廢—— 失敗重試必須沿用同一個值，因為「失敗」有可能只是回應在路上掉了、 訂單其實已經建立。每次重試都換新值的話，使用者按兩次就會買到兩份。
 */
...
/**
 * 重新試算。 失敗時**清掉明細而不是顯示錯誤**：試算只是預覽， 它掛掉不該擋住使用者結帳——真正的金額本來就是下單當下才決定的。 這是 fail-open，因為這道防線失守的代價只是「少看到折扣」。
 */
/**
 * 合併同一批變動。 進頁面時購物車、優惠券、預設地址是各自 resolve 的，每一份到齊都會觸發 一次試算——實測單次載入打了 **3 次** `/checkout/preview`。 三次的輸入不同（地址還沒設好時算不出運費），所以不是重複請求而是 三次不同的試算，但使用者只看得到最後一次。
 */
...
/**
 * 序號，用來丟掉**過期**的試算結果。 沒有它的話：使用者從本島換到離島，兩次試算同時在路上， 而先發出的本島那份比較慢回來——它會覆蓋掉離島的結果， 畫面顯示 80 元運費而下單當下收 200 元。 這與去抖是兩件事，去抖減少請求，序號保證顯示的是最後一次的答案。
 */
let previewSeq = 0
