...
/**
 * 撰寫評價。 **可評項目由後端算**（`/reviewable`），前端不自己比對訂單行與既有評價—— 前端再實作一次的話，症狀會是「畫面說可以評，送出卻被拒絕」。 一次只評一項。做成「一次評完整張訂單」看起來省事， 但那會讓其中一項失敗時整個表單卡住，而使用者不知道是哪一項有問題。 逐項送出則每一項的成敗都是獨立的，失敗的那一項還留在清單上可以重試。
 */
