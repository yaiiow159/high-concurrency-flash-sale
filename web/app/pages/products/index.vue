...
/** 已載入的商品。 第一頁來自 SSR（可被 ISR 快取），後續由「載入更多」在客戶端接上去。 **不把後續頁併進 SSR**——那會讓捲到第幾頁變成快取鍵的一部分， 命中率立刻掉到零。 */
...
/**
 * 逐層展開，不是一次攤平整棵樹。 先前只攤平兩層，商品卻掛在第三層，於是 225 個類目只有 15 個點得到—— 而那 15 個又因為篩選不含子樹全部回空頁面（ADR-0022）。 **但「完整攤平」是另一個極端**：225 個類目一次全列出來， 實測會把商品格擠出整個畫面，使用者要捲過一整頁的類目才看得到第一件商品。 攤平修好了「點不到」，卻換來「找不到」。 因此只顯示：根類目 + 目前選取路徑上每一層的子類目。 沒選時只有根，逐層點下去逐層展開，畫面上最多幾十個而不是 225 個。 選取狀態本身就在網址上，不需要額外的展開狀態。
 */
...
/**
 * 評分。 **一次批次取回整頁的商品，不是每張卡各打一次**——一頁 24 件商品 逐件查就是 24 次往返，而那正是 N+1 在前端的樣子。 在客戶端載入而不是併進這一頁的 SSR：這一頁是 ISR 快取 5 分鐘的， 評分跟著一起被快取的話，新評價要等快取過期才看得到。 這與「庫存不進這一頁」是同一個判斷——變動頻率不同的資料不該共用快取。 失敗時整份留空，卡片就不顯示星等。fail-open：評分掛掉不該讓人逛不了商品。
 */
