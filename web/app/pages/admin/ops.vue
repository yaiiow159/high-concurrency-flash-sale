...
/**
 * 維運工具。 這一頁放的是**成本高或不可逆**的操作，因此它與其他後台頁的排版不同： 每一個動作各佔一張卡、先寫清楚它會做什麼、按鈕在最後。 工作佇列式的密集列表在這裡是錯的——那種版面鼓勵連續按下去。 對帳與修復刻意分成兩個按鈕。「先看再修」是唯一安全的順序， 而把它們合成一個「檢查並修復」會讓人在還不知道差異是什麼之前就動手。
 */
...
/**
 * 重建索引。 **二次確認，而且訊息要說出成本**：它會把整個商品表重新寫進 Elasticsearch。 商品少的時候幾秒就好，多的時候是一段可觀的時間與寫入量—— 而使用者無法從按鈕上看出這件事。
 */
