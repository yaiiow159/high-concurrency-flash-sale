...
/**
 * 後台總覽。 **這一頁只回答一個問題：現在有什麼事等著我做。** 不放營業額、不放折線圖——那些該從 Grafana 看，那裡已經有 Prometheus 的資料， 在後台重畫一次只是多一份會過時的實作（ADR-0015「不做的事」）。 每一張卡都是**待辦數量 + 一個入口**。數字為 0 時仍然顯示， 而不是把卡片藏起來——「今天沒有待出貨」本身就是維運想知道的資訊， 而消失的卡片會讓人懷疑是不是壞了。
 */
...
/**
 * 三個查詢並行。它們互不相依，串起來只是把延遲加成三倍。 **各自 catch**：對帳掛掉不該讓「待出貨有幾筆」也看不到。 失敗的那一格顯示破折號，而不是 0——「查不到」與「沒有」是兩件事。
 */
async function load() {
  loading.value = true
