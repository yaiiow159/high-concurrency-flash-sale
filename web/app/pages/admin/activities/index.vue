...
/**
 * 秒殺活動。 這一頁的每個動作都比商品管理危險一階，因為它們牽動的是**庫存**： - 上架：立刻開放搶購 - 下架：立刻擋住新的搶購（已成立的訂單與已扣的庫存不受影響） - 預熱：把庫存寫進 Redis 餘量是**當下的 Redis 值，不經快取**。維運剛下架一檔活動、 後台卻因為快取還顯示上架中，他會再按一次——而那才是真正危險的地方。
 */
...
/**
 * 預熱。 **不提供 `force`。** 覆寫既有餘量是維運補救手段， 誤按的代價是把賣掉的量重新放出來賣一次。要用它就用 curl—— 那道摩擦力本身就是保護措施（ADR-0015 決策 6 的延伸）。
 */
