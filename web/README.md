...
| 庫存（分頁在背景） | 暫停 | 回到前景立刻補抓一次。開賣前先開好分頁放著的人很多，他們看不到數字卻照樣在打端點 |
...
訂單頁認得這個狀態（`processing: true`），會顯示「已搶到，訂單建立中」與排隊資訊，
而不是一張空訂單。

### 重整不會變成第二次搶購

`requestId` 與受理後的 `orderNo` 存在 sessionStorage。送出途中重整或斷線，
下一次按下去沿用同一個 `requestId`（後端的冪等鍵）；已受理的則直接接回輪詢。
只有**明確被拒**才作廢——網路錯誤與 5xx 的語意是「不知道送到沒」，此時換新值等於發第二筆請求。
