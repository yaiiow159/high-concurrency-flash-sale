...
而且不只是「重複投遞」——`auto-offset-reset` 設成 `earliest`，
代表**新的 consumer group 第一次上線時會重放整個 topic**。
會員積分的消費端上線時就這樣追溯處理了所有歷史訂單；
那次結果是好的（唯一索引擋住重複），但它是副作用不是決定。

因此新增消費端時要問的是兩個問題：
「重複投遞會怎樣」與「**把歷史全部重跑一次會怎樣**」。
第二題的答案若不是「沒事」，那個動作就不該放在消費端裡
（寄信、扣款、呼叫外部 API 都屬於這一類）。
...
### 6-1. 新增 `@Scheduled` 時，跨節點互斥是必答題

單節點跑得好好的排程，副本一開到兩個就變成同一件事做兩次。
**問法與鐵則 4 完全相同**：「同時被跑兩次會怎樣？」

答案不是「沒事」的話，就要用 `distributedLock.tryExecuteWithLock` 包起來
（參考 `StockReleaseScheduler`）。**包住的範圍必須涵蓋整批做完，而不只是撈取那一下**——
提前釋放的話，另一個節點會接手同一批還沒標記完成的資料。

它擋的是「**同時執行**」，不是「每輪只有一個節點跑」——鎖在動作結束就釋放。
另外 `tryExecuteWithLock` 的 `leaseTime` 參數目前的實作**不會使用**
（Redisson 看門狗自動續期），調它不會有任何效果。

`NotificationDeliveryScheduler` 漏掉這件事：撈取與標記已寄之間隔著一次真正的寄信，
兩個節點會讓使用者收到兩封一樣的信，而紀錄上只有一筆，完全看不出異常。

例外只有一種——**每個節點各自要做一次的事**（如 `SnowflakeNodeIdGuard.renew`
續的是自己的租約）。這種反而不可以加鎖，加了會讓沒搶到鎖的節點租約過期。

### 6-2. 多副本時 `snowflake.node-id` 必須各自不同

預設值是 0，所以「多開一個副本卻忘記改設定」是零阻力就會發生的事，
後果是兩個節點在同一毫秒發出**完全相同的訂單號**。

`SnowflakeNodeIdGuard` 會在啟動時向 Redis 宣告編號，撞號就**拒絕啟動**
（fail-closed：誤判很吵但一改設定就好，漏判會安靜到尖峰才炸）。
Redis 連不上時放行——那時這個節點本來就做不了秒殺。
...
- **一到兩句就好，點出核心問題即可**。不要寫多段落 Javadoc、不要加 `<h2>` 小標、
  不要把實測數字與完整推理寫進註解。**那些屬於 ADR 與 PR 描述**，
  在那裡有人會讀、也會跟著決策一起更新；寫在程式碼裡只會變成沒人維護的長文
